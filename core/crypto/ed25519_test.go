...
func TestBasicSignAndVerify(t *testing.T) {
	pri, pub, err := GenerateEd25519Key(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("PrivateKey", func(t *testing.T) {
		for name, f := range map[string]func() ([]byte, error){
			...
			t.Run(name, func(t *testing.T) {
				...
				privNew, err := ProtoUnmarshalPriKey(bts)
				if err != nil {
					t.Fatal(err)
				}
	...
	t.Run("PublicKey", func(t *testing.T) {
		for name, f := range map[string]func() ([]byte, error){
			...
			t.Run(name, func(t *testing.T) {
				...
				pubNew, err := ProtoUnmarshalPubKey(bts)
				if err != nil {
					t.Fatal(err)
				}
...
func TestUnmarshalErrors(t *testing.T) {
	t.Run("PublicKey", func(t *testing.T) {
		t.Run("Invalid data length", func(t *testing.T) {
			...
			if _, err := ProtoUnmarshalPubKey(data); err == nil {
	...
	t.Run("PrivateKey", func(t *testing.T) {
		t.Run("Redundant public key mismatch", func(t *testing.T) {
			pri, _, err := GenerateEd25519Key(rand.Reader)
			if err != nil {
				t.Fatal(err)
			}
		...
		t.Run("Invalid data length", func(t *testing.T) {
			...
			_, err = ProtoUnmarshalPriKey(data)
			if err == nil {
				t.Fatal("expected an error")
			}
		})
